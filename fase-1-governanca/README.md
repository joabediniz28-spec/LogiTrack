...
*   **Membros Simulados:** `ana.silva` (CEO), `carlos.moraes` (CTO)
...
*   **Membros Simulados:** `julia.costa` (Backend Dev), `lucas.almeida` (Frontend Dev)
...
*   **Membros Simulados:** `bruno.santos` (Cloud Engineer), `mariana.lima` (SecOps)
...
*   **Membros Simulados:** `rodrigo.dias` (CFO)
*   **Função:** Monitoramento de orçamentos, auditoria de gastos e emissão de relatórios de faturamento.

---

## 🛡️ Matriz de Permissões (Permission Sets)

As políticas de acesso foram desenhadas utilizando **AWS Managed Policies** associadas diretamente aos grupos do IAM Identity Center:

| Nome do Grupo | Permission Set Aplicado | Justificativa de Segurança (Least Privilege) |
| :--- | :--- | :--- |
| **`LogiTrack-CLevel`** | `ReadOnlyAccess` | Permite visualizar toda a infraestrutura para auditoria sem permissão de escrita/exclusão. |
...
| **`LogiTrack-FinOps`** | `Billing` | Acesso exclusivo ao *AWS Billing and Cost Management*. Membros do grupo são completamente bloqueados para interagir com serviços técnicos (Lambda, S3, DynamoDB). |
